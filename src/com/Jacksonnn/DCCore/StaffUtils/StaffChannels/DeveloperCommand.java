    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender.hasPermission(GeneralMethods.ChatModes.DEVELOPER.getChatPerm())) {
            if (args.length == 0) {
                ...
                String chatprefix = ChatColor.translateAlternateColorCodes('&', GeneralMethods.translateColorCodes(ConfigManager.langConfig.get().getString("Language.StaffChats.Developer.Prefix")));
                String msgColor = ChatColor.translateAlternateColorCodes('&', GeneralMethods.translateColorCodes(ConfigManager.langConfig.get().getString("Language.StaffChats.Developer.msgColor")));

                Bukkit.broadcast(chatprefix + sender.getName() + ": " + msgColor + ChatColor.translateAlternateColorCodes('&', GeneralMethods.translateColorCodes(StringUtils.join(args, ' '))), GeneralMethods.ChatModes.DEVELOPER.getChatPerm());
            }
        } else {
