    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender.hasPermission(GeneralMethods.ChatModes.STAFF.getChatPerm())) {
            if (args.length == 0) {
                ...
                Bukkit.broadcast(chatprefix + sender.getName() + ": " + msgColor + ChatColor.translateAlternateColorCodes('&', GeneralMethods.translateHEXColorCode(StringUtils.join(args, ' '))), GeneralMethods.ChatModes.STAFF.getChatPerm());
            }
        } else {
